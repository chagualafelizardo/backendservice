...
export const getAllUsers = async (req, res) => {
  try {
    ...
    const response = users.map(user => {
      const imgBase64 = user.img ? user.img.toString('base64') : null; // Certifique-se de que 'img' é um Buffer
      return {
        ...user.toJSON(),
        imgBase64: imgBase64, // Adiciona o campo 'imgBase64' à resposta JSON
      };
    });
    res.json(response);
  } catch (error) {
